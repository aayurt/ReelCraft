...
function getNextAccountPath() {
  ...
  if (nextIndex >= accounts.length) {
    nextIndex = 0; // Restart from account1
  }
...
async function generateVideoFromImage(imagePath, prompt, outputName, authStatePath, onRateLimit = null) {
  ...
  try {
    ...
    // Check for and close "Qwen Studio" modal if it appears
    ...
    // The user found .mode-select-open works
    ...
    await page.waitForTimeout(2000); // Wait for mode to switch

    // In Qwen, we often need to click 'Upload attachment' from the + menu again
    // Trigger the file upload
    console.log('Triggering file upload dialog...');

    // We try to trigger the file chooser by clicking the upload button
    // The user suggested #uploadButton, but we'll also look for the '+' menu as a fallback
    const [fileChooser] = await Promise.all([
      page.waitForEvent('filechooser'),
      (async () => {
        // Try the user's selector first
        const userButton = page.locator('#uploadButton');
        if (await userButton.isVisible()) {
          await userButton.click();
        } else {
          // Fallback to the '+' menu flow we verified earlier
          await page.click('.mode-select-open');
          await page.waitForTimeout(1000);
    ...
    // Wait for the UI to reflect the upload (look for a thumbnail or removal of upload placeholder)
    console.log('Waiting for upload to be processed by UI...');
    await page.waitForTimeout(20000);

    // Take a screenshot to verify upload state
    await page.screenshot({ path: `verify/upload_verify_${outputName.split('.')[0]}.png` });

    // Enter the prompt
    console.log(`Entering prompt: ${prompt}...`);
    // Selector from debug DOM: .message-input-textarea
    ...
    while (Date.now() - startTime < TIMEOUT) {
      // Look for the specific Qwen video container
      ...
      if (videoFound) {
        ...
        // Search for the specific toolbar item with "Download" text
    ...
    // Close the modal using the specific close button class
...
if (require.main === module) {
  ...
  if (!fs.existsSync(img)) {
    console.log('Usage: node qwen_automate.js <image_path> <prompt> <output_name> [auth_account|auth_path|--reset|--list]');
    ...
    console.log('Example: node qwen_automate.js input.jpg "motion prompt" video.mp4');
    console.log('Example: node qwen_automate.js input.jpg "motion prompt" video.mp4 3');
    console.log('Example: node qwen_automate.js input.jpg "motion prompt" video.mp4 --reset');
